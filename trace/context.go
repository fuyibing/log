...
func New() context.Context {
	return context.WithValue(context.Background(),
		TracingKey, NewTracing().WithRoot(),
	)
}
...
func Child(ctx context.Context, text string, args ...interface{}) context.Context {
	if tracing, ok := ctx.Value(TracingKey).(*Tracing); ok {
		return context.WithValue(ctx, TracingKey, NewTracing().WithTracing(tracing))
	}
	return New()
}
...
func FromRequest(request *http.Request) context.Context {
	if s := request.Header.Get(TracingTraceId); s != "" {
		return context.WithValue(context.Background(), TracingKey, NewTracing().WithRequest(request))
	}
	return New()
}
